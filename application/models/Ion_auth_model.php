...
class Ion_auth_model extends CI_Model {
    ...
    public function login($identity, $password, $remember = FALSE) {
        ...
        $this->trigger_events('extra_where');
        $this->db->select('u.*,ju.namajabatan,hu.*,j.idjabatan,p.namakaryawan')
                ->from ('users u')
                ->join('jabatanuser ju', 'ju.nik = u.username') 
                ->join('jabatan j', 'ju.namajabatan = j.namajabatan') 
                ->join('hakakses hu', 'hu.idjabatan = j.idjabatan')     
                ->join('karyawan p', 'p.nik = u.username')     
                ->where('u.username', $identity)
                ->where('u.passwords', md5($_POST['password']))
                ->limit(1)
                ->order_by('idusers', 'desc');
        $query = $this->db->get();
        
        if ($query->num_rows() === 1) {
            $user = $query->row();

            //$password = md5($password);//$this->hash_password_db($user->id, $password);
    ...
    public function cekUser($identity,$nohp){
         $this->db->select('*')
                ->from ('karyawan ')
                ->where('nik', $identity)
                ->where('nohp', $nohp);
        ...
        if (isset($groups)) {
            ...
            */
            ...
            /*if (!empty($group_names)) {
                $this->db->join($this->tables['tb_groups'], $this->tables['users_groups'] . '.' . $this->join['tb_groups'] . ' = ' . $this->tables['tb_groups'] . '.id', 'inner');
                $this->db->where_in($this->tables['tb_groups'] . '.name', $group_names);
            }
            if (!empty($group_ids)) {
                $this->db->{$or_where_in}($this->tables['users_groups'] . '.' . $this->join['tb_groups'], $group_ids);
            }*/
        }
    ...
    /*public function get_users_groups($id = FALSE) {
        ...
        return $this->db->select($this->tables['users_groups'] . '.' . $this->join['tb_groups'] . ' as id, ' . $this->tables['tb_groups'] . '.name, ' . $this->tables['tb_groups'] . '.description')
                        ->where($this->tables['users_groups'] . '.' . $this->join['users'], $id)
                        ->join($this->tables['tb_groups'], $this->tables['users_groups'] . '.' . $this->join['tb_groups'] . '=' . $this->tables['tb_groups'] . '.id')
    ...
    */
    ...
    public function add_to_group($group_ids, $user_id = false) {
        ...
        foreach ($group_ids as $group_id) {
            if ($this->db->insert($this->tables['users_groups'], array($this->join['tb_groups'] => (float) $group_id, $this->join['users'] => (float) $user_id))) {
    ...
    public function remove_from_group($group_ids = false, $user_id = false) {
        ...
        if (!empty($group_ids)) {
            ...
            foreach ($group_ids as $group_id) {
                $this->db->delete($this->tables['users_groups'], array($this->join['tb_groups'] => (float) $group_id, $this->join['users'] => (float) $user_id));
    ...
    public function set_session($user) {
        ...
        $session_data = array(
            ...
            'idusers' => $user->idusers, //everyone likes to overwrite id so we'll use user_id
    ...
    public function delete_group($group_id = FALSE) {
        ...
        $this->db->delete($this->tables['users_groups'], array($this->join['tb_groups'] => $group_id));
